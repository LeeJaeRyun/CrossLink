...
def check_company_special(row):
    """
    [FEEDBACK 2026-01-21]
    「会社名に特殊記号」判定は、就業先企業名だけでなく紹介元会社名も対象にする
    """
    ...
    # 就業先会社名 or 紹介元会社名 に特殊記号があればNG
    if any(mark in work for mark in SPECIAL_COMPANY_MARKS):
        return "NG", "就業先会社名に特殊記号(㈱等)を含む"
    if intro != "" and any(mark in intro for mark in SPECIAL_COMPANY_MARKS):
        return "NG", "紹介元会社名に特殊記号(㈱等)を含む"
...
def check_job_title(row):
    ...
    # (B-2) [FEEDBACK 2026-01-21] 管理記号の可能性: 数字を含む => 要確認
    # 例: 「経験者採用1」
    if re.search(r"\d", v):
        return "要確認", "職種に数字が含まれる(管理記号の可能性)"

    # (C) [FEEDBACK 2026-01-21] 施設名判定は一旦候補から外す
...
print("✅ 職種: 地域名/地名形式はNG、条件は要確認（施設名判定は一旦除外）")