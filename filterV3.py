...
# CSV 로드 (인코딩 자동 감지: utf-8-sig → cp932 → utf-8)
df = None
last_err = None
...
def check_intro_company_special(row):
    """紹介元会社名に特殊記号が含まれているかチェック"""
    v = safe_strip(row.get(col_intro_company))
    if v == "":
        return "OK", ""  # 空欄は許可(別の関数でチェック)
    if any(mark in v for mark in SPECIAL_COMPANY_MARKS):
...
def check_job_title(row):
    ...
    # (C) 数字が含まれている => 要確認 (管理番号など職種関係ない文字列)
    if re.search(r'\d', v):
        return "要確認", "職種に数字が含まれる(管理番号等の可能性)"
